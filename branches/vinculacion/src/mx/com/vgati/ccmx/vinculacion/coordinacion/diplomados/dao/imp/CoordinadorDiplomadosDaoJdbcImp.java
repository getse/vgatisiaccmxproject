...
public class CoordinadorDiplomadosDaoJdbcImp extends AbstractBaseJdbcDao
	...
	public List<PyMEs> getPymes() throws DaoException {
		...
		return getJdbcTemplate().query(query.toString(), new PymesRowMapper());
	}
	...
	@SuppressWarnings("rawtypes")
	public class MenuAniosDiploamdosResultSetExtractor implements
			ResultSetExtractor {
			...
			if (sesiones.get(i).getDomicilios() != null
				...
				if (!updateDomicilios(sesiones.get(i).getDomicilios())) {
					return new Mensaje(1,
							"Error al guardar sesiones, intentelo mas tarde.");
				}
	...
	public int insertDomicilios(Domicilios domicilios) throws JdbcDaoException {
	...
	public Mensaje saveAsistencias(List<Participantes> participantes)
		...
		for (int i = 0; i < participantes.size(); i++) {
			if (participantes.get(i).getIdSesion1() > 0) {
				if (hasRegistroSesion(participantes.get(i).getId(),
						participantes.get(i).getIdSesion1())) {
					query = new StringBuffer();
			...
			if (participantes.get(i).getIdSesion2() > 0) {
				if (hasRegistroSesion(participantes.get(i).getId(),
						participantes.get(i).getIdSesion2())) {
					query = new StringBuffer();
					query.append("UPDATE INFRA.ASISTENCIAS SET ASISTENCIA=");
			...
			if (participantes.get(i).getIdSesion3() > 0) {
				if (hasRegistroSesion(participantes.get(i).getId(),
						participantes.get(i).getIdSesion3())) {
					query = new StringBuffer();
					query.append("UPDATE INFRA.ASISTENCIAS SET ASISTENCIA=");
			...
			if (participantes.get(i).getIdSesion4() > 0) {
				if (hasRegistroSesion(participantes.get(i).getId(),
						participantes.get(i).getIdSesion4())) {
					query = new StringBuffer();
					query.append("UPDATE INFRA.ASISTENCIAS SET ASISTENCIA=");
	...
	public Mensaje saveFacturas(List<Participantes> participantes)
		...
		return new Mensaje(
				0,
				"Las modificaciones han sido guardadas correctamente, en un momento enviaremos el correo de solicitud.");
	}
	...
	public Mensaje saveInasistententes(Participantes p, int idDiplomado)
			throws DaoException {
		StringBuffer query = new StringBuffer();
		int idServicioDiplomado = getIdServicioDiplomado(p.getIdUsuario(),
				idDiplomado);
		if (idServicioDiplomado == 0) {
			...
			query = new StringBuffer();
			idServicioDiplomado = getIdServicioDiplomado(p.getIdUsuario(),
					idDiplomado);
		}
		if (p.isAsistencia1()) {
		...
		try {
			...
			return new Mensaje(
					0,
					"Se han guardado las modificaciones correctamente, en seguida se enviaran las notificaciones correspondientes.");
		} catch (Exception e) {
			log.fatal("Error al guardar los cambios, " + e);
			return new Mensaje(1,
					"No es posible reasignar los Asistentes seleccionados,"
	...
	@SuppressWarnings("unchecked")
	public int getIdSesion(int idServiciosDiplomado, int sesion)
			throws DaoException {
		StringBuffer query = new StringBuffer();
	...
	@SuppressWarnings("unchecked")
	public int getIdServicioDiplomado(int idUsuario, int idDiplomado)
			throws DaoException {
		StringBuffer query = new StringBuffer();
		...
		try {
			...
			if (lista != null && lista.length > 0) {
				for (int i = 0; i < lista.length; i++) {
					...
					query.append(" )");
					log.debug("inscribirPymes(" + i + ") " + query + " ID "
							+ idDiplomado + " - " + lista[i]);
					getJdbcTemplate().update(query.toString());
				}
			}
		} catch (Exception e) {
			log.fatal("ERROR al insertar las PyMEs, " + e);
			return new Mensaje(1,
					"No es posible inscribir las PyMEs, intentelo más tarde.");
		}
		return new Mensaje(0, "Las PyMEs fueron insritas correctamente.");
	}
}