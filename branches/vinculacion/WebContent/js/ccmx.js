...
function validacion(sesion){
	...
	if( diplomado.length == 0 || /^\s+$/.test( diplomado ) ){
		...
		alert("Ingrese el nombre del diplomado");
		return false;
	} else if (idExpositor == null || idExpositor.length == 0 || /^\s+$/.test(idExpositor)) {
		...
		return false;
	} else if (ingreso == null || ingreso.length == 0 || /^\s+$/.test(ingreso)) {
		...
		return false;
	} else if (idInstuctor == null || idInstuctor.length == 0 || /^\s+$/.test(idInstuctor)) {
		...
		return false;
	} else if (delegacion == null || delegacion.length == 0 || /^\s+$/.test(delegacion)) {
		...
		return false;
	} else if (codigoPostal == null || codigoPostal.length == 0 || /^\s+$/.test(codigoPostal)) {
